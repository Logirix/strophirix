(function (global, factory) {
  ...
  Strophe.Connection.prototype = {
    ...
    sendPresence: function sendPresence(elem, callback, errback, timeout) {
      ...
      if (!this.disconnecting && this.connected) this.send(elem);
      return id;
    },
    ...
    */
    ...
    _attemptSASLAuth: function _attemptSASLAuth(mechanisms) {
      ...
      for (var i = 0;i < mechanisms.length;++i) {
        ...
        if (!this.disconnecting && this.connected) this.send(request_auth_exchange.tree());
    ...
    _sasl_challenge_cb: function _sasl_challenge_cb(elem) {
      ...
      if (!this.disconnecting && this.connected) this.send(stanza.tree());
      return true;
    },
    ...
    _attemptLegacyAuth: function _attemptLegacyAuth() {
      if (Strophe.getNodeFromJid(this.jid) === null) {
        ...
        if (!this.disconnecting && this.connected) this.send($iq({
          'type': "get",
          'to': this.domain,
          'id': "_auth_1"
        }).c("query", {
    ...
    _onLegacyAuthIQResult: function _onLegacyAuthIQResult(elem) {
      ...
      if (!this.disconnecting && this.connected) this.send(iq.tree());
      return false;
    },
    ...
    _onIdle: function _onIdle() {
      ...
      if (this.connected) {
        this._idleTimeout = setInterval(function () {
          return idleContent();
        }, 100);
      }
  ...
  Strophe$2.Websocket.prototype = {
    ...
    _disconnect: function _disconnect(pres) {
      if (this.socket && this.socket.readyState !== WebSocket.CLOSED) {
        ...
        try {
          if (this.socket && this.socket.readyState === WebSocket.OPEN && this.socket.send) {
            this.socket.send(closeString).catch((e) => console.error)
          } else {
      ...
      if (data.length > 0 && !this._conn.paused) {
        for (var i = 0;i < data.length;i++) {
          if (data[i] !== null) {
            ...
            try {
              if (this.socket && this.socket.readyState === WebSocket.OPEN && this.socket.send) {
                this.socket.send(rawStanza).catch((e) => console.error)
              } else {
                Strophe$2.warn("Couldn't send rawStanza");
              }
            } catch (e) {
              Strophe$2.warn("Couldn't send rawStanza");
            }
    ...
    _onOpen: function _onOpen() {
      ...
      try {
        if (this.socket && this.socket.readyState === WebSocket.OPEN && this.socket.send) {
          this.socket.send(startString).catch((e) => console.error)
        } else {
          Strophe$2.warn("Couldn't send <start /> tag.");
        }
      } catch (e) {
        Strophe$2.warn("Couldn't send <start /> tag.");
      }
